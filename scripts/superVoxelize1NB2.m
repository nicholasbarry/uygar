...
if exist([dirname,'/',imgFilename,'_1_runParameters.mat'],'file')
    ...
    superVoxelOpts.spatialDistanceCalculationOpts.upperBound = 10;         % orig at 31, try 10-15, big impact on runtime (in calculate_sAff)
    superVoxelOpts.colorDistanceUpperBound                   = 0.03;

    superVoxelOpts.splitHardSVopts.detThreshold              = 1e-9;       % increasing to maybe 10e-10, for initial checks (runtime). controls homogeneity of svoxels
    superVoxelOpts.splitHardSVopts.connectivity              = 26;
    superVoxelOpts.splitHardSVopts.subdivisionSizeThreshold  = 100;        % increasing this will bring down runtime (fewer SVs will be split/created the higher it gets)
    ...
    superVoxelOpts.removeSmallComponents.minVoxelCount       = 100;        % higher = faster (in removeSmallComponents). test at 100.
    ...
    % save run parameters
    save([dirname,'/',imgFilename,'_1_runParameters.mat'],'start','dirname','CHANNELCOUNT','sigma','superVoxelOpts','clusterCount','unitTest','graphData','numXpixels','numYpixels','numSlices','stackHeight','imgFilePath','-v7.3')
end

%% ingest, denoise, normalize
...
if exist([dirname,'/',imgFilename,sprintf('_2_denoised_bm4d_colors%d_sigma%d.mat',CHANNELCOUNT,sigma)],'file')
    ...
    fprintf(' done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
    if isequal(unique(thisVol),unique(bbVol))
        fprintf('\ndenoising failed!\n')
    end
    clear thisVol i;
    ...
    % checkpoint
    save([dirname,'/',imgFilename,sprintf('_2_denoised_normalized_bm4d_colors%d_sigma%d.mat',CHANNELCOUNT,sigma)], 'bbVol', '-v7.3')
    fprintf(' done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
end

%% watershed

if exist([dirname,'/',imgFilename,'_3_watershed.mat'],'file')
    load([dirname,'/',imgFilename,'_3_watershed.mat'])
else
    ...
    * for the moment using a dumb thresholding measure
    ...
    % simpler method for assigning background
    fprintf('\nAssigning background... \n');tic
    [L1] = find_background(L0,unique_regions);
    fprintf('\                         done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start));
    fprintf('\nSaving L1...\n\n')
    save([dirname,'/',imgFilename,'_3_watershed.mat'],'L1','-v7.3')
    fprintf('               done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
...
if exist([dirname,'/',imgFilename,'_4_assign6.mat'],'file')
    load([dirname,'/',imgFilename,'_4_assign6.mat'])
else
    stackSize = size(L1);
    numVoxels = numel(L1);
    ...
    try % gets rid of boundaries; L2: 1 = background, 2...max(L2(:)) = watershed regions
        L2 = assign6BoundariesNB_mex(int32(L1), bbVol); % exceeds intmax() when compiling on openmind ? not sure if there's a workaround? double? vs int32?
    catch
    ...
    % checkpoint
    fprintf('\nSaving L2...\n\n')
    save([dirname,'/',imgFilename,'_4_assign6.mat'], 'L2', 'stackSize','numVoxels','-v7.3')
end


%% filter out small supervoxels
if exist([dirname,'/',imgFilename,'_5_mask.mat'],'file')
    load([dirname,'/',imgFilename,'_5_mask.mat'])
else
    ...
    fprintf(' done in %0.2f seconds, total time: %s.\n',toc,char(datetime-start))
    fprintf('\nSaving mask...\n\n');tic
    save([dirname,'/',imgFilename,'_5_mask.mat'],'mask','-v7.3')
    fprintf('                 done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
end


%% store supervoxels 
if exist([dirname,'/',imgFilename,'_6_superVoxelCells.mat'],'file')
    load([dirname,'/',imgFilename,'_6_superVoxelCells.mat'])
else

    L3        = L2;
    L3(~mask) = 1;
    regions   = unique(L3);
    fprintf('\nForming %d supervoxels...',numel(regions)-1);tic            % takes ~30-45min
    ...
    save([dirname,'/',imgFilename,'_6_superVoxelCells.mat'],'superVoxelCells','-v7.3')
    fprintf('                            done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
end


%% split heterogeneous supervoxels
if exist([dirname,'/',imgFilename,'_7_split_superVoxelCells.mat'],'file')
    load([dirname,'/',imgFilename,'_7_split_superVoxelCells.mat'])
else
    ...
    superVoxelCells = splitHardSuperVoxels_kmeansNB3(superVoxelOpts.splitHardSVopts, superVoxelCells, bbVol); 
    ...
    save([dirname,'/',imgFilename,'_7_split_superVoxelCells.mat'],'superVoxelCells','-v7.3')
    save([dirname,'/',imgFilename,'_1_runParameters.mat'],'superVoxelOpts','-append')
    fprintf('                                  done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
end

%% get supervoxel means, boundaries
if exist([dirname,'/',imgFilename,'_8_superVoxelBoundaries.mat'],'file')
    load([dirname,'/',imgFilename,'_8_superVoxelBoundaries.mat'])
else
    % takes ~6min
    ...
    save([dirname,'/',imgFilename,'_8_superVoxelBoundaries.mat'], 'cc', 'superVoxelMeans', 'boundaryVoxelsSub', '-v7.3')
    fprintf('                                  done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
end

%% calculate supervoxel affinities
if exist([dirname,'/',imgFilename,'_9_superVoxelAffinities.mat'],'file')
    load([dirname,'/',imgFilename,'_9_superVoxelAffinities.mat'])
else
    % get affinity graph
    fprintf('\nCalculating supervoxel affinities, upperbound = %d...\n',superVoxelOpts.spatialDistanceCalculationOpts.upperBound);tic
    sAff = calculate_sAff(cc, boundaryVoxelsSub, superVoxelOpts.spatialDistanceCalculationOpts); 
    fprintf('\nCalculating supervoxel affinities, upperbound = %d... done in %0.2f seconds, total time: %s.\n\n',superVoxelOpts.spatialDistanceCalculationOpts.upperBound,toc,char(datetime-start))
    ...
    save([dirname,'/',imgFilename,'_9_superVoxelAffinities.mat'], 'sAff','cc','square_sAff', '-v7.3');
end


%% initial clustering
if exist([dirname,'/',imgFilename,'_10_clustering.mat'],'file')
    load([dirname,'/',imgFilename,'_10_clustering.mat'])
else
    fprintf('\nClustering...');tic
    ...
    fprintf(' done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
    % checkpoint
    save([dirname,'/',imgFilename,'_10_clustering.mat'],'superVoxelMeansLUV','index','superVoxelOpts','-v7.3');
...
segProjectionWrite
fprintf('                         done in %0.2f seconds, total time: %s.\n\n',toc,char(datetime-start))
save([dirname,'/',imgFilename,'_11_forManipulation.mat'],'index','graphData','superVoxelMeansLUV','square_sAff','stackSize','superVoxelCells','numXpixels','numYpixels','-V7.3');
...
%% this might work?
system(sprintf('cp $output %s/$output',dirname));
